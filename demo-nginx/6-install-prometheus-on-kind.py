...
pyinfra -y -vv --user root ${SERVER_NAME} 6-install-prometheus-on-kind.py
"""
...
def add_prometheus_repo() -> None:
    ...
    server.shell(
        ...
        commands=[f"helm repo add prometheus-community {REPO}", "helm repo update"],
    )
...
# def deploy_custom_prometheus_rules():
#     """Deploys custom PrometheusRule objects."""
#     print("--- Deploying Custom Prometheus Rules ---")
#     current_env = os.environ.copy()
#     current_env["KUBECONFIG"] = "/root/.kube/config"

#     # Define the PrometheusRule manifest for test-rules
#     prometheus_rule_manifest = """
# apiVersion: monitoring.coreos.com/v1
# kind: PrometheusRule
# metadata:
#   name: custom-test-rules
#   namespace: monitoring # Assuming Prometheus is installed in 'monitoring' namespace
#   labels:
#     # These labels must match the ruleSelector of your Prometheus instance
#     # For kube-prometheus-stack, common labels are often release: prometheus
#     prometheus: kube-prometheus-stack
#     role: alert-rules
# spec:
#   groups:
#   - name: smo-alerts
#     rules: [] # User specified empty rules for now
# """
