...
pyinfra -y -vvv --user root ${SERVER_NAME} 13-demo-bxl.py
"""
...
def install_hello_world():
    server.shell(
        ...
        commands=[(f"yes | {SMO_CLI} remove hello-world-graph || true")],
        _get_pty=True,
    )
    server.shell(
        name="Remove prior hello-world-graph",
    ...
    server.shell(
        ...
        commands=[
            (f"cd {TOP_DIR} && . .venv/bin/activate && cd 1-brussels-demo && inv all")
        ],
        _get_pty=True,
    )
    result = server.shell(
