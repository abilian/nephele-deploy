"""
Minimal recipe build for the resilience app.

Warning: connection as user root.

pyinfra -y -v --user root ${SERVER_NAME} 12-deploy-demo-resilience.py
"""
...
DEMO = f"{GITS}/h3ni-demos/4-resilience-demo"
PROJECT = "demo4"
GRAPH_NAME = "resilience-critical-graph"
...
def clean_installed_graphs() -> None:
    for name in (
        "hello-world-graph",
        "image-detection-graph",
        "resilience-critical-graph",
    ):
...
def change_hdag_file() -> None:
    files.put(
        name="Put hdag file",
        src="hdag.yaml",
        dest="/root/gits/h3ni-demos/4-resilience-demo/hdag/hdag.yaml",
        mode="644",
    )


def prepare_resilience() -> None:
    ...
    server.shell(
        ...
        commands=[
            ...
            cd {DEMO}/critical-service/templates
    ...
    result = server.shell(
        ...
        commands=[
            ...
            docker build -t "127.0.0.1:5000/critical-service-0.1.0" src/critical-service

            docker push "127.0.0.1:5000/critical-service-0.1.0"

            cp -a hdag build/
            cp -a critical-service build/

            hdarctl package tar -d dist build/hdag
            hdarctl package tar -d dist build/critical-service

            hdarctl push dist/resilience-critical-graph-0.1.0.tar.gz http://127.0.0.1:5000/demo4
            hdarctl push dist/critical-service-0.1.0.tar.gz http://127.0.0.1:5000/demo4
...
def deploy_on_smo() -> None:
    ...
    result = server.shell(
        name=f"Deploy {GRAPH_NAME} to smo",
        commands=[cmd],
        _get_pty=True,
        _shell_executable="/bin/bash",
    )
...
def find_demo4() -> None:
    result = server.shell(
        name="Find demo4 in member1",
        commands=[
            """
            export KUBECONFIG="/root/.kube/karmada-apiserver.config"
            kubectl config use-context member1

            pod_name=$(kubectl get pods -n demo4 -o jsonpath='{.items[0].metadata.name}')
            kubectl wait --for=condition=Ready pod/$pod_name -n demo4 --timeout=120s
            kubectl port-forward -n demo4 $pod_name 8080:8080 &
            sleep 2
            counter=0
            code=""
    ...
    python.call(
        name="Show demo4 response in member1",
        function=log_callback,
        result=result,
    )
